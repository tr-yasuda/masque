...
impl Session {
    ...
    ///
    /// Note that an unset `SETTINGS_H3_DATAGRAM` value is treated as disabled
    /// per RFC 9297 Section 2.1.1, so the capsule fallback can be selected once
    /// `Capsule-Protocol` has been mutually agreed, without requiring an
    /// explicit `SETTINGS_H3_DATAGRAM` advertisement.
    #[must_use]
